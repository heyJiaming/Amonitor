...
//        _table.backgroundView = nil;
  //      _table.backgroundColor = [UIColor clearColor];
        _table.separatorColor = BLUEFONTCOLOR;
...
- (instancetype)initWithBounds:(CGRect)bounds titleMenus:(NSArray *)titles
{
    ...
    if (self) {
        ...
        if ([_table respondsToSelector:@selector(setSeparatorInset:)]) {
            //让线头不留白
            [_table setSeparatorInset:UIEdgeInsetsMake(0, 4, 0, 4)];
         
            
        }
        if ([_table respondsToSelector:@selector(setLayoutMargins:)]) {
            
            [_table setLayoutMargins:UIEdgeInsetsMake(0, 4, 0, 4)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor = BLUEFONTCOLOR;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell respondsToSelector:@selector(setSeparatorInset:)]) {
        ...
        [cell setSeparatorInset:UIEdgeInsetsMake(0, 4, 0, 4)];
        
    }
    
    if ([cell respondsToSelector:@selector(setLayoutMargins:)]) {
        
        [cell setLayoutMargins:UIEdgeInsetsMake(0, 4, 0, 4)];
